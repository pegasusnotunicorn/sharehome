import type { Character, CharacterWithNav } from "../../types/character";
...
export function getRandomPerson(notThisPerson?: string): Character {
  const rando = allCharacters[Math.floor(Math.random() * allCharacters.length)];
...
export function getSpecificPerson(personName: string): Character | false {
  //search for them
...
export function getSpecificPersonByURL(urlName: string): CharacterWithNav | false {
  //search for them
  for (let i = 0; i < allCharacters.length; i++) {
...
export function getAllFinishedPeople(random?: boolean): Character[] {
  const removedIgnoredChars = allCharacters.filter((e) => {
...
export function getAllCharacters(random?: boolean): Character[] {
  if (random) {
