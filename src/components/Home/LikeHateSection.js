...
const LikeHateSection = (props) => {
  ...
  useLayoutEffect(()=>{
    ...
    }).fromTo([arrowsContainerRef.current], {
      opacity:1,
    },{
      opacity:1,
      duration:duration,
    ...
    <div id={props.id} className={props.className}>
      <div className="likehateLeftContainer likehateContainers">
        <GsapFadeScrub fadeIn className="imageAndArrowContainer">
          <img className={`tableSplash ${characterCoords[currentChar].name}`} src="/images/illustrations/small_splash.jpg" alt="dinner table" />
...
const CharacterArc = ({ goodTrait, badTrait, chaoticTrait, coords, showLeft }) => {
      ...
      { showLeft &&
        <div className="arrowWrapper flipped">
          <GsapWiggle degree={2} className="arrowWrapperSVG">
            <img className="arrowTailSVG" src="/images/icons/arrowtail.svg" alt="part of the arrow" />
            <img className="arrowHeadSVG" src="/images/icons/arrowhead.svg" alt="part of the arrow" />
          </GsapWiggle>
        </div>
      }
