...
import { randomNum } from "../utils/useMath";
...
const getRandomXY = (): [number, number] => {
  return [
    randomNum(15, 85),
...
export const ShootingStar = (props: ShootingStarProps) => {
  ...
  const createRandomXYTweens = useRef<() => void>(() => {
    tl.current!.fromTo(
      randomXRef.current,
      {
  ...
  const repeatTl = () => {
    ...
    (tl.current as any).repeatDelay = Math.random() * 2; //set new repeat delay
  ...
  useLayoutEffect(() => {
    tl.current = gsap.timeline({
      delay: Number(delay.current) + 0.25,
      repeat: -1,
