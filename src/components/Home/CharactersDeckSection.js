...
const CharactersDeckSection = (props) => {
  ...
  for ( var x = 0 ; x < numSections ; x++ ){
    splitIntoSections.push(allCharacters.slice(x * charactersPerRow, (x * charactersPerRow) + charactersPerRow));
  }
...
const Decks = (props) => {
  ...
  useLayoutEffect(() => {
    ...
    if (width > 1200 && width <= 1600) { distanceRanges = [4,12]; }
      ...
      isEvenMultTemp *= -1;
      if (rowIndex === 1) finalY += randomNum(70,80);
      if (rowIndex === 0) finalY += 25;
    }

    //final X and Y coordinates
    let finalX = `${isEvenMultTemp * randomNum(distanceRanges[0], distanceRanges[1])}vw`;
    finalY = `${finalY}%`

    if (tl){
      cards.forEach((curr, index, array) => {
        tl.killTweensOf(cardRefs.current[index]);
        tl.fromTo(cardRefs.current[index], {
          x: "-200%",
          y: 0,
          ...
          rotationZ:(rowIndex === 0) ? "10deg" : (rowIndex === 1) ? "-10deg" : "2deg",
        });
      });
    }
    return () => {
  ...
  //return the cards
  return props.cards.map((curr, index, array) => {
